...
########### Data Constants ###########
DATA_DIR = '../Datasets/cluster-trace-gpu-v2020/'
if not os.access('../Figures/DataExploration', os.F_OK):
    os.mkdir('../Figures/DataExploration')
if not os.access('../Figures/DataExploration', os.W_OK):
    print('Cannot write to ../Figures/DataExploration, please fix it.')
    exit()
else:
    print('figures saved to ../Figures/DataExploration')
...
def get_dfia(dfi):
    dfi_s = dfi[dfi.start_time > 0][['job_name', 'task_name', 'start_time']].groupby(
        ['job_name', 'task_name']).min()  # start_time
    dfi_e = dfi[dfi.end_time > 0][['job_name', 'task_name', 'end_time']].groupby(
        ['job_name', 'task_name']).max()  # end_time
    ...
    dfi_u = dfi[['job_name', 'task_name', 'status']].drop_duplicates().groupby(
        ['job_name', 'task_name']).max()  # status
    dfia = dfi_u
...
def get_dfa(dft, dfj, dfi, dfg):
    ...
    dfa = dfa.merge(dfg[[x for x in dfg.columns if x != 'user']], on='inst_id',
                    how='left')  # reserve NaN ones by how='left'
...
def get_dfwitm(dfwit, csv_file='intermediate_data/machine_metric_shennong_machine_all.csv'):
    ...
    dfwitm = dfwit.merge(res_df.loc[:, ~res_df.columns.isin(['start_time', 'end_time', 'machine'])], on='worker_name',
                         how='left')
    return dfwitm
...
def plot_data_cdf(data, inverse=False, datalabel=None, xlabel=None, title=None, xlog=False, xlim=None, ylog=False,
                  xticks=None, figsize=(4, 3), dpi=120, savefig=None, ylabel=None):
...
def plot_data_cdfs(data, datalabel=None, inverse=False, xlabel=None, title=None, xlog=False, ylog=False, xticks=None,
                   figsize=(4, 3), dpi=120, xlim=None, ylim=None, ylabel=None, yticks=None, savefig=None, loc='best',
                   fontsize=None):
    plt.figure(figsize=figsize, dpi=dpi)
...
def plan_minus_usg_over_cap_task(dfas):
    ...
    dfas_task = dfas.groupby(['job_name', 'task_name'])[
        ['plan_gpu_minus_usage_over_capacity', 'plan_cpu_minus_usage_over_capacity',
         'plan_mem_minus_usage_over_capacity']].mean()
